        ...
        $.get(url, function (data) {
            if (data.error) {
                ...
                data.stationsArray.forEach(el => {
                    options += "<option value='" + el.id + "'>" + el.stationName + "</option>";
                })
    ...
    $.get(url, function (data) {
        ...
        if (routesArray.length == 0) {
            ...
            schedulesArray.forEach(el => {
                options += "<option value='" + el.id + "'>" + el.scheduleName + " - " + el.text + "</option>";
            });
            ...
            routesArray.forEach(el => {
                if (el.id == routeId) {
                    fare = el.routeFare;
                    options += "<option value='" + el.id + "' selected>" + el.routeName + "</option>";
                } else {
                    options += "<option value='" + el.id + "'>" + el.routeName + "</option>";
                }
            });
...
$('#tDate').on('change', function () {
    ...
    if (memberIds.length == 0) {
        ...
        var url = "getTrains/" + memberCount + "/" + isAc + "/" + routeId + "/" + scheduleId + "/" + date;
        ...
        $.get(url, function (data) {
            if (data.error) {
                ...
                trainArray.forEach(el => {
                    options += "<option value='" + el.id + "'>" + el.trainName + "</option>";
                });
